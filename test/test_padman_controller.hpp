...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_PADMAN_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_PADMAN_CONTROLLER_HPP_
...
using ControllerStateMsg = padman_controller::PadmanController::ControllerStateMsg;
using ControllerReferenceMsg = padman_controller::PadmanController::ControllerReferenceMsg;
using ControllerModeSrvType = padman_controller::PadmanController::ControllerModeSrvType;
...
class TestablePadmanController : public padman_controller::PadmanController
{
  FRIEND_TEST(PadmanControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(PadmanControllerTest, activate_success);
  FRIEND_TEST(PadmanControllerTest, reactivate_success);
  FRIEND_TEST(PadmanControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(PadmanControllerTest, test_update_logic_fast);
  FRIEND_TEST(PadmanControllerTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = padman_controller::PadmanController::on_configure(previous_state);
...
class PadmanControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_padman_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_padman_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_padman_controller")
  {
    ASSERT_EQ(
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_padman_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestablePadmanController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_PADMAN_CONTROLLER_HPP_
